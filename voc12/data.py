...
class VOC12ClsDataset(VOC12ImageDataset):
    ...
    def __getitem__(self, idx):
        ...
        return name, img, label,idx

class VOC12ClsDatasetseg(VOC12ImageDataset):
    ...
    def load_png(self,p):
        labelseg = np.asarray(Image.open(p), dtype=np.int32)
        labelseg = labelseg.astype(np.int64)
        return labelseg
    def get_label(self,pred_path, idx):
        name, img = super().__getitem__(idx)
        label_path = os.path.join(self.voc12_root, 'SegmentationClassAug', name + '.png')
        pred_pathi=os.path.join(pred_path,name + '.png')
        labelseg=self.load_png(label_path)
        pred=self.load_png(pred_pathi)
        return labelseg,pred

    def __getitem__(self, idx):
        name, img = super().__getitem__(idx)
        label_path = os.path.join(self.voc12_root, 'SegmentationClassAug', name + '.png')
        labelseg = np.asarray(Image.open(label_path), dtype=np.int32)
        labelseg = labelseg.astype(np.int64)

        label = torch.from_numpy(self.label_list[idx])

        return name, img, label,idx,labelseg
...
class VOC12ClsDatasetMSF(VOC12ClsDataset):
    ...
    def __getitem__(self, idx):
        name, img, label,idx= super().__getitem__(idx)
...
class VOC12ClsDatasetMSFseg(VOC12ClsDatasetseg):
    ...
    def __getitem__(self, idx):
        name, img, label,idx,labelseg= super().__getitem__(idx)
        ...
        return name, msf_img_list, label,labelseg

class VOC12ClsDatasetMSFseg1(VOC12ClsDatasetseg):
    ...
    def __getitem__(self, idx):
        ...
        return name, msf_img_list, label,labelseg,h,w


class VOC12ClsDatasetMSFseg2(VOC12ClsDatasetseg):
    ...
    def __getitem__(self, idx):
        ...
        if self.inter_transform:
            for i in range(len(msf_img_list)):
                msf_img_list[i] = self.inter_transform(msf_img_list[i])
        # h,w = np.asarray(img).shape[:2]
        return name, msf_img_list, label,labelseg
