...
## Usage
#### 1. Train a classification network to get CAMs which has 512 channels before GAP.

```bash
python3 train_cls.py 
```

#### 2. Train a classification network with class 、subclass label based on the model  with 512 channels before GAP

```bash
python3 train_subcls512.py
```

#### 3. Train a classification network with class 、subclass label based on the model  with 4096 channels before GAP

```bash
python3 train_subcls4096.py
```


#### (Optional) Check the accuracy of CAMs（512 channel model）.
```bash
python3 infer_clsa512.py --infer_list voc12/val.txt  --weights res138k
```

#### (Optional) Check the accuracy of CAMs (4096 channel model).
```bash
python3 infer_clsa4096.py --infer_list voc12/val.txt  --weights res38k
```


## Original AffinityNet Results and Trained Models
