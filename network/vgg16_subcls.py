...
class Net(network.vgg16d.Net):
    ...
    def __init__(self):
        ...
        self.fc8sub = nn.Conv2d(1024, 20*10, 1, bias=False)
        torch.nn.init.xavier_uniform_(self.fc8sub.weight)
        self.not_training = [self.conv1_1, self.conv1_2,
                             self.conv2_1, self.conv2_2]
        self.from_scratch_layers = [self.fc8,self.fc8sub]

    def forward(self, x):
        x = super().forward(x)
        x = self.drop7(x)
        xf=F.avg_pool2d(x, kernel_size=(x.size(2), x.size(3)), padding=0)
        xp = self.fc8(x)
        xsub=self.fc8sub(x)
        xsub = F.avg_pool2d(xsub, kernel_size=(x.size(2), x.size(3)), padding=0)
        xp = F.avg_pool2d(xp, kernel_size=(x.size(2), x.size(3)), padding=0)
        xsub=xsub.view(-1,20*10)
        xp = xp.view(-1, 20)
